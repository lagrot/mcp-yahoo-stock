...
async def test_get_current_price_fallback_to_history(mock_ticker: MagicMock) -> None:
    ...
    mock_df = pd.DataFrame(
        {"Close": [145.0, 146.0]}, index=[pd.Timestamp.now(), pd.Timestamp.now()]
    )
...
async def test_is_cache_fresh() -> None:
    ...
    aware_fresh = (now - datetime.timedelta(hours=12)).isoformat()
...
async def test_get_history_cache_hit() -> None:
    ...
    with patch(
        "src.data.yfinance_client._get_history_from_cache", new_callable=AsyncMock
    ) as mock_cache:
...
async def test_get_history_cache_miss_api_success() -> None:
    ...
    with patch(
        ...
        with patch(
            "src.data.yfinance_client._fetch_history_from_api", new_callable=AsyncMock
        ) as mock_api:
